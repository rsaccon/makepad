...
impl Cx {
            ...
            Area::All => {
                for window_id in 0..self.windows.len() {
                    let redraw = match self.windows[window_id].window_state {
                        CxWindowState::Create { .. } | CxWindowState::Created => true,
                        _ => false,
                    };
                    if redraw {
    ...
    pub fn redraw_pass_and_dep_of_passes(&mut self, pass_id: usize) {
        ...
        loop {
            if let Some(main_view_id) = self.passes[walk_pass_id].main_view_id {
                self.redraw_parent_area(Area::View(ViewArea {
                    redraw_id: 0,
    ...
    pub fn check_ended_anim_areas(&mut self, time: f64) {
        ...
        loop {
            ...
            if anim_start_time.is_nan() || time - anim_start_time >= anim_total_time {
                self.ended_anim_areas
                    .push(self.playing_anim_areas.remove(i));
            } else {
                i = i + 1;
            }
        }
    ...
    pub fn update_area_refs(&mut self, old_area: Area, new_area: Area) {
        ...
        if let Some(anim_anim) = self
            .playing_anim_areas
            .iter_mut()
            .find(|v| v.area == old_area)
        {
    ...
    pub fn set_color(&mut self, name: &str, val: Color) {
        self.style_values
            .insert(name.to_string(), StyleValue::Color(val));
    }

    pub fn set_font(&mut self, name: &str, val: &str) {
        self.style_values
            .insert(name.to_string(), StyleValue::Font(val.to_string()));
    }

    pub fn set_size(&mut self, name: &str, val: f64) {
        self.style_values
            .insert(name.to_string(), StyleValue::Size(val));
    }
        ...
        F: FnMut(&mut Cx, &mut Event),
    {
        ...
        if self.last_key_focus != self.key_focus {
            ...
            event_handler(
                self,
                &mut Event::KeyFocus(KeyFocusEvent {
                    last: last_key_focus,
        ...
        F: FnMut(&mut Cx, &mut Event),
    {
        self.call_event_handler(
            &mut event_handler,
            &mut Event::Animate(AnimateEvent {
                time: time,
        ...
        if self.ended_anim_areas.len() > 0 {
            self.call_event_handler(
                &mut event_handler,
                &mut Event::AnimateEnded(AnimateEvent {
                    time: time,
                    frame: self.repaint_id,
                }),
            );
