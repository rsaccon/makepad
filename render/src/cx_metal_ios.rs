...
impl Cx {
            ...
            if sub_view_id != 0 {
                ...
                if draw_call.instance_dirty {
                    ...
                    draw_call
                        .platform
                        .inst_vbuf
                        .update_with_f32_data(metal_cx, &draw_call.instance);
                }
                ...
                if let Some(buf) = &draw_call.platform.inst_vbuf.multi_buffer_read().buffer {
                    encoder.set_vertex_buffer(1, Some(&buf), 0);
                } else {
                ...
                encoder.set_vertex_bytes(
                    2,
                    (cxuniforms.len() * 4) as u64,
                    cxuniforms.as_ptr() as *const std::ffi::c_void,
                );
                encoder.set_vertex_bytes(
                    3,
                    (cxview.uniforms.len() * 4) as u64,
                    cxview.uniforms.as_ptr() as *const std::ffi::c_void,
                );
                encoder.set_vertex_bytes(
                    4,
                    (draw_call.uniforms.len() * 4) as u64,
                    draw_call.uniforms.as_ptr() as *const std::ffi::c_void,
                );
    ...
    pub fn setup_render_pass_descriptor(
        &mut self,
        render_pass_descriptor: &RenderPassDescriptorRef,
        pass_id: usize,
        dpi_factor: f32,
        first_texture: Option<&metal::TextureRef>,
        metal_cx: &MetalCx,
    ) {
        ...
        if let Some(depth_texture_id) = self.passes[pass_id].depth_texture {
            ...
            if self.passes[pass_id].platform.mtl_depth_state.is_none() {
                ...
                self.passes[pass_id].platform.mtl_depth_state =
                    Some(metal_cx.device.new_depth_stencil_state(&desc));
            }
        }
    }
    ...
    pub fn draw_pass_to_layer(
        ...
        if let Some(drawable) = layer.next_drawable() {
            ...
            self.setup_render_pass_descriptor(
    ...
    pub fn draw_pass_to_texture(&mut self, pass_id: usize, dpi_factor: f32, metal_cx: &MetalCx) {
        let view_id = self.passes[pass_id].main_view_id.unwrap();
...
impl MetalCx {
    ...
    pub fn update_platform_render_target(
        ...
        let width = if let Some(width) = cxtexture.desc.width {
            width as u64
        } else {
            (size.x * dpi_factor) as u64
        };
        let height = if let Some(height) = cxtexture.desc.height {
            height as u64
        } else {
            (size.y * dpi_factor) as u64
        };
...
impl MetalWindow {
    ...
    pub fn resize_core_animation_layer(&mut self, _metal_cx: &MetalCx) -> bool {
        ...
        if self.cal_size != cal_size {
            ...
            self.core_animation_layer
                .set_drawable_size(CGSize::new(cal_size.x as f64, cal_size.y as f64));
            self.core_animation_layer
                .set_contents_scale(self.window_geom.dpi_factor as f64);
...
impl MetalBuffer {
    ...
    pub fn update_with_f32_data(&mut self, metal_cx: &MetalCx, data: &Vec<f32>) {
        ...
        if let None = &elem.buffer {
            elem.buffer = Some(metal_cx.device.new_buffer(
