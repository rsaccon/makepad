...
impl CocoaApp {
    pub fn new() -> CocoaApp {
        unsafe {
            ...
            CocoaApp {
                const_attributes_for_marked_text: NSArray::arrayWithObjects(
                    nil,
                    &vec![
    ...
    pub fn post_signal(signal_id: usize, value: usize) {
        ...
        //     (*post_delegate_instance)
        //         .set_ivar("cocoa_app_ptr", GLOBAL_COCOA_APP as *mut _ as *mut c_void);
    ...
    pub fn stop_timer(&mut self, timer_id: u64) {
        for i in 0..self.timers.len() {
            if self.timers[i].timer_id == timer_id {
                unsafe { msg_send![self.timers[i].nstimer, invalidate] }
    ...
    pub fn send_timer_received(&mut self, nstimer: id) {
        ...
        //                 data2: 0u64
        //             ];
        //             //rs//  msg_send![appkit::NSApp(), postEvent: nsevent atStart: 0];
        //             let _: () = msg_send![pool, release];
        //         }
        //         return;
        //     }
    ...
    pub fn send_signal_event(&mut self, signal_id: usize, value: usize) {
        self.do_callback(&mut vec![Event::Signal(SignalEvent {
            signal_id: signal_id,
            value: value,
        })]);
    }
...
impl CocoaWindow {
    ...
    pub fn get_inner_size(&self) -> Vec2 {
        ...
        // Vec2 {
        //     x: view_frame.size.width as f32,
        //     y: view_frame.size.height as f32,
        // }
    ...
    pub fn get_outer_size(&self) -> Vec2 {
        // let window_frame = unsafe { NSWindow::frame(self.window) };
        // Vec2 {
        //     x: window_frame.size.width as f32,
        //     y: window_frame.size.height as f32,
        // }
        Vec2 { x: 100.0, y: 100.0 }
    }
...
pub fn define_cocoa_timer_delegate() -> *const Class {
    ...
    extern "C" fn received_live_resize(this: &Object, _: Sel, _nstimer: id) {
        // let ca = get_cocoa_app(this);
        // ca.send_paint_event();
    }
    ...
    unsafe {
        decl.add_method(
            sel!(receivedTimer:),
            received_timer as extern "C" fn(&Object, Sel, id),
        );
        decl.add_method(
            sel!(receivedLiveResize:),
            received_live_resize as extern "C" fn(&Object, Sel, id),
        );
    }
...
pub fn define_cocoa_post_delegate() -> *const Class {
    extern "C" fn received_post(this: &Object, _: Sel, _nstimer: id) {
        // let ca = get_cocoa_app(this);
        // unsafe {
    ...
    unsafe {
        decl.add_method(
            sel!(receivedPost:),
            received_post as extern "C" fn(&Object, Sel, id),
        );
    }
    // Store internal state as user data
    decl.add_ivar::<*mut c_void>("cocoa_app_ptr");
...
pub fn define_cocoa_window_class() -> *const Class {
    ...
    let window_superclass = class!(UIWindow);
